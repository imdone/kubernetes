...
// TODO: This is created only to reduce number of changes in a single PR. id:3820 gh:3835
...
func getManualConversionFunctions(context *generator.Context, pkg *types.Package, manualMap conversionFuncMap) {
	...
	for _, f := range pkg.Functions {
		...
		// TODO: This should call the Namer directly. id:3963 gh:3984
...
func Packages(context *generator.Context, arguments *args.GeneratorArgs) generator.Packages {
	...
	// TODO: This is too ad-hoc.  We need a better way. id:4060 gh:4080
	...
	// defined.
	// TODO: in the future, relax the nested manual conversion requirement id:3597 gh:3612
	...
	for _, i := range context.Inputs {
		...
		// TODO: build a more fundamental concept in gengo for dealing with modifications id:4007 gh:4027
		...
		// Otherwise, the generator will create the file in the wrong location
		// in the output directory.
		// TODO: build a more fundamental concept in gengo for dealing with modifications id:3821 gh:3836
		// to vendored packages.
...
func (e equalMemoryTypes) equal(a, b *types.Type, alreadyVisitedTypes map[*types.Type]bool) bool {
	...
	case in.Kind == out.Kind:
		...
		case types.Interface:
			// TODO: determine whether the interfaces are actually equivalent - for now, they must have the id:3964 gh:3985
			// same type.
			return false
		case types.Builtin:
...
func (g *genConversion) convertibleOnlyWithinPackage(inType, outType *types.Type) bool {
	...
	// TODO: Consider generating functions for other kinds too. id:4061 gh:4081
	if t.Kind != types.Struct {
		return false
	}
...
func (g *genConversion) doMap(inType, outType *types.Type, sw *generator.SnippetWriter) {
	...
	if isDirectlyAssignable(inType.Key, outType.Key) {
		...
		// TODO: Implement it when necessary. id:4008 gh:4028
		sw.Do("for range *in {\n", nil)
		sw.Do("// FIXME: Converting unassignable keys unsupported $.|raw$\n", inType.Key) id:3822 gh:3838
	}
	sw.Do("}\n", nil)
}
	...
	if inType.Elem == outType.Elem && inType.Elem.Kind == types.Builtin {
		...
		if isDirectlyAssignable(inType.Elem, outType.Elem) {
			...
			if function, ok := g.preexists(inType.Elem, outType.Elem); ok {
				...
				// TODO: This triggers on metav1.ObjectMeta <-> metav1.ObjectMeta and id:3965 gh:3986
